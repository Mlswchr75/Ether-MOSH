...
export function StickerForge() {
  ...
  const forge = async () => {
    ...
    try {
      ...
      const width = selected.asset.width ?? imageData.width, height = selected.asset.height ?? imageData.height, nameBase = selected.asset.name || "MOSH Sticker";
      let json: any;
      if (chosen === "vector") {
  ...
  return <div className="pointer-events-auto relative">
    <button type="button" disabled={!selected} onClick={() => setOpen(v => !v)} className="flex items-center gap-1.5 rounded-full border border-violet-300/25 bg-black/70 px-2.5 py-2 font-mono text-[8px] uppercase tracking-[0.12em] text-violet-100 backdrop-blur-md disabled:opacity-25"><WandSparkles size={11}/> Forge</button>
    {open && <div className="absolute bottom-11 left-1/2 z-[120] w-[min(92vw,22rem)] -translate-x-1/2 rounded-2xl border border-violet-300/20 bg-black/95 p-3 shadow-2xl backdrop-blur-xl">
      ...
      {selected && (selected.asset.kind === "lottie-json" || selected.asset.kind === "dotlottie") && <p className="mt-2 text-center font-mono text-[7px] uppercase text-white/30">Selected overlay is already Lottie.</p>}
    </div>}
  </div>;
}
