...
import { OverlayOwnFxParams } from "./OverlayOwnFxParams";

const TRACK_OPTIONS: Array<{ value: "off" | OverlayTrackingTarget; label: string }> = [
  { value: "off", label: "Track off" }, { value: "hand", label: "Hand" }, { value: "face", label: "Face" },
  { value: "person", label: "Subject" }, { value: "object", label: "Object" }, { value: "journey", label: "Journey focus" },
];
...
export function OverlayInspector() {
  ...
  const setTracking = (target: "off" | OverlayTrackingTarget) => patchEntity(entity.id, { tracking: target === "off" ? null : { enabled: true, target, offsetX: 0, offsetY: 0, scaleWithTarget: target === "person" || target === "hand" || target === "face", rotateWithTarget: target === "hand" || target === "face" } });
  const setCompositing = (mode: OverlayCompositingMode) => {
    if (mode === "own-fx" && entity.ownFx.length === 0 && DEFAULT_OWN_FX) { patchEntity(entity.id, { compositing: mode, ownFx: [makeLayer(DEFAULT_OWN_FX)] }); return; }
  ...
  return <div className="pointer-events-auto absolute left-3 top-3 z-[90] flex max-w-[calc(100vw-1.5rem)] flex-wrap items-center gap-1 rounded-xl border border-white/15 bg-black/75 p-1.5 shadow-xl backdrop-blur-md">
    ...
    <label className="flex items-center gap-1 rounded-full border border-white/10 px-2 py-1 text-white/55" title="Compositing route"><Layers3 size={9} /><select aria-label="Sticker compositing route" value={entity.compositing} onChange={e => setCompositing(e.target.value as OverlayCompositingMode)} className="bg-transparent font-mono text-[7px] uppercase text-inherit outline-none"><option value="after-fx">After FX</option><option value="before-fx">Before FX</option><option value="own-fx">Own FX</option></select></label>

    <OverlayLottieControls entity={entity} />

    {entity.compositing === "own-fx" && <div className="flex flex-wrap items-center gap-1 rounded-lg border border-fuchsia-300/20 bg-fuchsia-500/5 p-1">
      <WandSparkles size={9} className="mx-1 text-fuchsia-200" />
      {entity.ownFx.map((layer, index) => <div key={layer.id} className="flex items-center gap-0.5 rounded-full border border-fuchsia-300/20 px-1 py-0.5 text-fuchsia-100">
        <select aria-label={`Own FX layer ${index + 1}`} value={layer.effectId} onChange={e => patchEntity(entity.id, { ownFx: replaceOwnFxLayer(entity.ownFx, layer.id, e.target.value) })} className="max-w-28 bg-transparent font-mono text-[7px] uppercase outline-none">{OWN_FX.map(effect => <option key={effect.id} value={effect.id}>{effect.name}</option>)}</select>
    ...
    <OverlayOwnFxParams entity={entity} />
