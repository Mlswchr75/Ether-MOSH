import { Eye, EyeOff, Lock, Unlock, Layers3, Crosshair } from "lucide-react";
...
export function OverlayInspector() {
  ...
  return (
    <div className="pointer-events-auto absolute left-3 top-3 z-[90] flex max-w-[calc(100vw-1.5rem)] flex-wrap items-center gap-1 rounded-xl border border-white/15 bg-black/75 p-1.5 shadow-xl backdrop-blur-md">
      ...
      <select
        aria-label="Sticker blend mode"
        value={entity.blend}
        onChange={event => patchEntity(entity.id, { blend: event.target.value as BlendMode })}
        className="rounded-full border border-white/10 bg-black/70 px-2 py-1 font-mono text-[7px] uppercase text-white/65 outline-none"
        title="Blend mode"
      >
      ...
      <label className="flex items-center gap-1 rounded-full border border-white/10 px-2 py-1 text-white/55" title="Compositing route">
        <Layers3 size={9} />
        <select
          aria-label="Sticker compositing route"
          value={entity.compositing}
          onChange={event => patchEntity(entity.id, { compositing: event.target.value as OverlayCompositingMode })}
          className="bg-transparent font-mono text-[7px] uppercase text-inherit outline-none"
        >
          <option value="after-fx">After FX</option>
          <option value="before-fx" disabled>Before FX · soon</option>
          <option value="own-fx" disabled>Own FX · soon</option>
        </select>
      </label>
