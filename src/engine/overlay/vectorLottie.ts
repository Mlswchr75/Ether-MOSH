...
function shapePath(points: [number, number][]) { return { a: 0, k: { c: true, v: points, i: points.map(() => [0,0]), o: points.map(() => [0,0]) } }; }
...
export function buildVectorStickerLottie(input: { name: string; width: number; height: number; shapes: TracedStickerShape[]; preset: StickerLottiePreset; durationSeconds?: number; fps?: number }) {
  ...
  const layers = input.shapes.map((shape, index) => ({
    ddd:0, ind:index+1, ty:4, nm:`${input.name} ${index+1}`, sr:1, ks:motion(input.preset,width,height,op), ao:0,
    shapes:[{ ty:"sh", ks:shapePath(shape.points), nm:"Path" },{ ty:"fl", c:{ a:0, k:[shape.color[0]/255,shape.color[1]/255,shape.color[2]/255,1] }, o:{ a:0, k:Math.round(shape.color[3]/255*100) }, r:1, nm:"Fill" },{ ty:"tr", p:{a:0,k:[0,0]}, a:{a:0,k:[0,0]}, s:{a:0,k:[100,100]}, r:{a:0,k:0}, o:{a:0,k:100}, sk:{a:0,k:0}, sa:{a:0,k:0}, nm:"Transform" }],
    ip:0, op, st:0, bm:0,
  }));
