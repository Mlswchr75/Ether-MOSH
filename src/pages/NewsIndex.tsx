...
export default function NewsIndex() {
  ...
  return <main className="news-page">
    ...
    <section className="news-feature">
      ...
      <aside>{latestNewsArticles.filter(a => a !== featured).map(article => <Link key={article.slug} to={newsArticlePath(article)}><img src={article.image} alt=""/><span><small>{article.category}</small><strong>{article.title}</strong><em>{article.readMinutes} min read</em></span></Link>)}</aside>
    </section>
    <section className="news-latest"><header><h2>Latest field reports</h2><span>{latestNewsArticles.length} launch transmissions</span></header><ol>{latestNewsArticles.map((article, index) => <li key={article.slug}><Link to={newsArticlePath(article)}><b>{String(index + 1).padStart(2, "0")}</b><time dateTime={article.publishedAt}>Aug 31, 2026</time><span>{article.category}</span><strong>{article.title}</strong><em>{article.readMinutes} min <ArrowRight/></em></Link></li>)}</ol></section>
    <section className="news-resources"><div><p className="news-kicker">Practical resources</p><h2>Take the useful shit with you.</h2><p>No email. No gate. No desperate funnel-shaped nonsense.</p></div>{latestNewsArticles.map(article => <a key={article.slug} href={article.downloadHref} download><Download/><span><strong>{article.effectName} field card</strong><small>Markdown · recipes + production notes</small></span></a>)}<a href="/news/downloads/effect-starter-recipes.csv" download><Download/><span><strong>Starter recipes</strong><small>CSV · three launch effects</small></span></a></section>
    <NewsFooter/>
  </main>;
}
