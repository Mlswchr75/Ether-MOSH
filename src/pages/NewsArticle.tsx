...
export default function NewsArticle() {
  ...
  const dispatchNumber = String(NEWS_ARTICLES.indexOf(article) + 1).padStart(3, "0");
  const subjectLabel = article.subjectKind === "effect" ? "Effect" : "Mode";
  const controlCount = effect?.params.length ?? article.parameters.length;
  const relatedEffects = article.relatedEffects.flatMap(id => {
    const relatedEffect = EFFECTS_BY_ID[id];
  ...
  const jsonLd = [
    ...
    { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
      { "@type": "ListItem", position: 1, name: "Home", item: "https://ether-mosh.online/" },
      { "@type": "ListItem", position: 2, name: "News + Updates", item: "https://ether-mosh.online/news" },
  ...
  return <main className="news-page news-article">
    ...
    <article>
      ...
      <section className="news-related"><p className="news-kicker">Keep breaking things</p><h2>Related effects</h2><nav aria-label="Related effects">{relatedEffects.map(relatedEffect => <Link key={relatedEffect.id} to={`/effects#${relatedEffect.id}`}><span>{relatedEffect.category}</span><strong>{relatedEffect.name}</strong><ArrowUpRight/></Link>)}</nav></section>
      {article.product && <aside className="news-sponsor"><span>Shameless product placement / paid partnership with ourselves</span><h2>{article.product.name}</h2><p>{article.product.copy}</p><a href={article.product.href} target="_blank" rel="sponsored noreferrer">View the thing we interrupted science for <ArrowUpRight/></a></aside>}
      ...
      <section className="news-faq"><p className="news-kicker">People also ask</p><h2>Questions, answered.</h2>{article.faqs.map(f => <details key={f.question}><summary>{f.question}</summary><p>{f.answer}</p></details>)}</section>
